...
- (void) setProjection {
	...
	Vector3 *zero = [self.graphicsDevice.viewport unproject:[Vector3 zero] projection:basicEffect.projection view:[Matrix identity] world:[Matrix identity]];	
	Vector3 *one = [self.graphicsDevice.viewport unproject:[Vector3 vectorWithX:1 y:1 z:0] projection:basicEffect.projection view:[Matrix identity] world:[Matrix identity]];	
	pointSize = [[Vector3 subtract:one by:zero] length] / 3;
}
...
- (void) drawPointAt:(Vector2 *)position color:(Color *)color layerDepth:(float)layerDepth {
	[self drawLineFrom:[Vector2 vectorWithX:position.x-pointSize y:position.y-pointSize]
					to:[Vector2 vectorWithX:position.x+pointSize y:position.y+pointSize] color:color layerDepth:layerDepth];	
}
